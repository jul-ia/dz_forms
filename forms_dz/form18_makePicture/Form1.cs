...
namespace form18_makePicture
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            {
                MessageBox.Show("File 'puzzle.bmp' is not found\n"+e.ToString(), "Собери картинку", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Close();
                return;
            }
            ...
            for (int i = 0; i < nw; i++)
            {
                for (int j = 0; j < nh; j++)
                {
                    ...
                    if (showNumbers && field[i, j] != 0)
                        g.DrawString(Convert.ToString(field[i, j]), new Font("Tahoma", 10, FontStyle.Bold), Brushes.Black,
                            i * cw + 5, j * ch + 5 + menuStrip1.Height);
                }
            }
        }
        ...
        {
            if (!(((Math.Abs(cx - ex) == 1) && (cy - ey == 0)) || ((Math.Abs(cy - ey) == 1) && (cw - ex == 0))))
                return;
        ...
        private void toolStripMenuItem2_Click(object sender, EventArgs e)   //about
        {
